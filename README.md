...
3. In **Project Settings → Environment Variables**, add (same as `.env`):
   ...
   - `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI=https://courier-and-logistics.vercel.app/api/v1/auth/google/callback`
   - `REDIS_URL=` (optional; leave empty to degrade gracefully)
   - `BACKEND_URL=https://courier-and-logistics.vercel.app`, `FRONTEND_URL=http://localhost:3000`, `NODE_ENV=production`
...
**Stripe webhook on production:** in the Stripe Dashboard → Developers → Webhooks, add `https://courier-and-logistics.vercel.app/api/v1/payments/webhook` and subscribe to `checkout.session.completed` and `checkout.session.async_payment_failed`. Copy the `whsec_...` signing secret into `STRIPE_WEBHOOK_SECRET`.

**Google OAuth on production:** in GCP OAuth Client, add `https://courier-and-logistics.vercel.app/api/v1/auth/google/callback` to **Authorized redirect URIs**.
